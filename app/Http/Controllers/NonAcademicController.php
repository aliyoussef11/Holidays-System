...
class NonAcademicController extends Controller
{
    public function index()
    {
        ...
        $january_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('january');
        ...
        //Get February Condition For Non academic user
        $february_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('february');
        ...
        //Get March Condition For Non academic user
        $march_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('march');
        ...
        //Get April Condition For Non academic user
        $april_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('april');
        ...
        //Get May Condition For Non academic user
        $may_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('may');
        ...
        //Get June Condition For Non academic user
        $june_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('june');
        ...
        //Get July Condition For Non academic user
        $july_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('july');
        ...
        //Get August Condition For Non academic user
        $august_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('august');
        ...
        //Get September Condition For Non academic user
        $september_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('september');
        ...
        //Get October Condition For Non academic user
        $october_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('october');
        ...
        //Get November Condition For Non academic user
        $november_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('november');
        ...
        //Get December Condition For Non academic user
        $december_condition = Condition::Where('role', '=', 'Non-Academic')->pluck('december');
        ...
        return view('non-academic.index', compact('events', 'calendar') ,['Remaining_holidays_per_year'=>$Remaining_holidays_per_year
    ...
    public function store(Request $request)
    {
        ...
        if($requests_number_for_current_user > 0){
            return redirect('/non-academic')->with('failed', 'There is a pending request, cannot send another one, Please Wait!');
        }
        else{     
        $current_date = Carbon::now()->format('Y-m-d');

        if($request->input('date') <= $current_date){
            return redirect('/non-academic')->with('failed', 'Your date is less than Our Date!');
        }
        else{
            ...
            if(in_array($request->input('date'), $dates)){
                return redirect('/non-academic')->with('failed', 'You are already booked this day as a holiday, Please Choose another one!'); 
            }
            else{
                $current_year = Carbon::now()->format('Y');
                ...
                if($number_of_date_same_year_for_same_user > $yearly_condition[0]){
                    return redirect('/non-academic')->with('failed', 'You reached The maximum number of holidays in this year!'); 
                }
                else{
                    ...
                    $condition_for_this_month =  Condition::Where('role', '=', 'Non-Academic')->pluck($monthName_entered);

                    if($count_holidays_perThisMonth >= $condition_for_this_month[0]){
                        return redirect('/non-academic')->with('failed', 'You reached The maximum number of holidays in this Month Entered!');
                    }
                    else{
                        $events = new Requests();
                        ...
                        return redirect('/non-academic')->with('success', 'Request Has been Sent!');
                    }
    ...
    public function display_edit_page(){
        ...
        return view('non-academic.edit-page', ['holidays'=> $holidays]);
    }

    public function show_edit_page_form($name, $role, $title, $date){
        return view('non-academic.show-edit-page-form', ['name'=>$name, 'role'=>$role ,'title'=> $title, 'date'=>$date]);
    }
    ...
    public function Edit_Holiday(Request $request, $name, $role, $title, $date){
        ...
        if($request->input('date') <= $current_date){
            return redirect('/edit-a-holiday-non-academic')->with('not-edited', 'Your date is less than Our Date!');
        }
        else{
            $dates_of_holidays = Holiday::where('name', '=', Auth::User()->name)->get(); 
            ...
            if(in_array($request->input('date'), $dates)){
                return redirect('/edit-a-holiday-non-academic')->with('not-edited', 'You are already booked this day as a holiday, Please Choose another one!'); 
            }
            else{
                    //Get Entered Month by the user
                    ...
                    if($Previous_month_date == $monthName_entered){
                        ...
                        if($count_holidays_perThisMonth >= $condition_for_this_month[0]){
                            return redirect('/edit-a-holiday-non-academic')->with('not-edited', 'You reached The maximum number of holidays in this Month Entered!');
                        }
                        else{
                            ...
                            return redirect('/non-academic')->with('success', 'Please wait for response!');
                        }
                    }
                    else{
                        //Get Number of holidays taken in the entered month 
...
public function display_delete_page(){
    $current_date = Carbon::now()->format('Y-m-d');
        ...
        return view('non-academic.delete-page', ['holidays'=> $holidays]);
}
...
public function Delete_Holiday($name, $role, $title, $date){
    ...
    if($deletion == true){
        return redirect()->guest('/non-academic')->with('success', 'Deleted Successfully!');
    }
    else{
        return redirect()->guest('/non-academic')->with('failed', 'Something went wrong!');
    }
}
...
public function Hide_Notification($name, $role, $title, $date, $response){
    ...
    if($deletion == true){
        return redirect()->guest('/non-academic');
    }
    else{
        return redirect()->guest('/non-academic')->with('failed', 'Something went wrong!');
    }
...
public function Display_Statistics(){
    ...
    return view('non-academic.Statistics', ['Remaining'=> $Remaining_holidays_per_year, 'Taken_Holidays' =>
    $count_Holidays]);
}
